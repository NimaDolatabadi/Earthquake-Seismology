...
c
      call nor2jhd(2,n,nrecord,nhead,data,stations,sta_list_count)
      n=n+1
      goto 10
c
 99   continue
...
c
c
      subroutine nor2jhd(unit,nevent,nrecord,nhead,data,stations,
     1 sta_list_count)
...
c
      ...
      read(data(1)(49:51),'(i3)') nstat 
      write(unit,'(1x,i3,1x,i3)') nevent, nstat
c
...
c         if(data(k)(11:11).eq.'P'.or.data(k)(11:11).eq.'S') then
         if(data(k)(11:11).eq.'P') then
c            read(data(k)(19:28),'(i2,i2,1x,f5.2)') hour,min,p_sec
            read(data(k)(19:28),'(i2,i2,f6.3)') hour,min,p_sec
...
c check for a matching S-phase at this station
         ...
         if(data(j)(11:11).eq.'S'.and.data(j)(2:5).eq.data(k)(2:5)) then
                  s_found=.true.
                  read(data(j)(19:28),'(i2,i2,1x,f5.2)') hour,min,s_sec
                  read(data(j)(15:15),'(i1)') s_weight
...
c write out the jhd format phase output
            if(s_found) then
               if(data(k)(5:5).eq.' ') then
                  write(unit,100) data(k)(2:4),p_sec,p_weight,
     1                 s_sec, s_weight
               else
                  write(unit,100) data(k)(2:5),p_sec,p_weight, 
     1                 s_sec, s_weight
               endif
            else
               if(data(k)(5:5).eq.' ') then
c                  write(unit,101) data(k)(2:4),p_sec,p_weight
                  write(unit,100) data(k)(2:4),p_sec,p_weight,
     1                 p_sec, 4
               else
c                  write(unit,101) data(k)(2:5),p_sec,p_weight
                  write(unit,100) data(k)(2:5),p_sec,p_weight, 
     1                 p_sec, 4
...
c
 ...
 100  format(1x,a4,1x,f6.2,1x,i1,1x,f6.2,1x,i1)
 101  format(1x,a4,1x,f6.2,1x,i1)
...
c      open(10,FILE=station_file,STATUS='OLD',err=98)
      ...
      do i=1,sta_list_count
         ...
         if (dec_lat.ne.0..and.dec_lon.ne.0.) then
           if(stations(i)(4:4).eq.' ') then
             write(3,100) stations(i)(1:3), dec_lat,dec_lon,elev
           else
             write(3,100) stations(i)(1:4), dec_lat,dec_lon,elev
           endif
         else
      ...
      enddo
 100  format(1x,a4,1x,f7.3,1x,f8.3,1x,f5.2,1x,'0',1x,'0')
c           
c         rewind(10)
c         do j=1,1000
c            read(10,'(a27)') line
