...
c	07 JAN 2005 - forces bwsac and bwsac2 to delete the file before writing. This is
c			required if the file already exists and is larger than necessary
c			Check the close(UNIT,status='delete') across compilers
c       09 May 2007 lot - added error check to open statements in brsac
c        5 Oct 2007 lot - added byte sawp to brsac
c       06 02 2013  jh  - fix a dimension, look for jh
c       22 02 2013  jh  - ----------------
c-----
	subroutine brsac (IRU,LN,name,data,swap,nerr)
c-----
...
c       swap    I*4     1 for byte swapping, 0 for not
...
c-----
	...
	real*4 rhdr(70),trhdr(70)
	integer*4 ihdr(40),tihdr(40)
	character*8 chdr(24)
	character*(*) name
        integer*4 swap
c-----
...
c-----
		if(ihdr(10).gt.LN)then
			...
			ihdr(10) = LN
			nerr = -2
                  goto 999 ! assume not sac file
		else 
			maxpts = ihdr(10)
			nerr = 0
		endif
                if (maxpts.le.0) goto 999
		nrec=632+4*maxpts
		nread = 0
c-----
C-----
...
C     &			access='direct',recl=nrec,err=999)
...
c----- 
		...
		if(nrec.gt.2048)then
			open (IRU,file=name,form='unformatted',
     &				access='direct',recl=2048,err=999)
			ndat1 = (2048 - 632) / 4
			irec = 1
			read (IRU,rec=irec,err=998) (trhdr(i), i=1,70),
     &					 (tihdr(i), i=1,40),
     &					 (chdr(i), i=1,24),
     &					 (data(i), i=1,ndat1)
			nread = nread + ndat1
 1000			continue
			...
			irec = irec + 1
			read (IRU,rec=irec,err=998) (data(i), i=nl,nh)
			nread = nread + (nh-nl+1)
 ...
 1001			continue
		close (IRU)
			...
			open (IRU,file=name,form='unformatted',
     &				access='direct',recl=nrec,err=999)
			read (IRU,rec=1,err=998) (trhdr(i), i=1,70),
     &					 (tihdr(i), i=1,40),
     &					 (chdr(i), i=1,24),
     &					 (data(i), i=1,ndat)
		close (IRU)
...
c     1	'EVLA    ', 'EVLO    ', 'EVEL    ', 'EVDP    ', 'FHDR40  ', ! changed lo
...
c-----
	...
	data (rstr(i),i=1,45)/
     ...
     1	'RESP4   ', 'RESP5   ', 'RESP6   ', 'RESP7   ', 'RESP8   ', 
     1	'RESP9   ', 'STLA    ', 'STLO    ', 'STEL    ', 'STDP    ', 
c     1	'EVLA    ', 'EVLO    ', 'EVEL    ', 'EVDP    ', 'FHDR40  ', 
     1	'EVLA    ', 'EVLO    ', 'EVEL    ', 'EVDP    ', 'MAG     ', 
     1	'USER0   ', 'USER1   ', 'USER2   ', 'USER3   ', 'USER4   ' /
	data (rstr(i),i=46,70)/
     1	'USER5   ', 'USER6   ', 'USER7   ', 'USER8   ', 'USER9   ', 
...
c	never greater than desired. Else the dregs of the first will remain
c-----
...
c			1337 Header variable does not exist
c-----
cjh	character strcmd*(*), strval*8
	character strcmd*(*), strval*(*)
	parameter (NEVAL=50)
	character*8 eval(NEVAL)
...
c-----
  ...
  100	continue
	...
	subroutine rsac1(infile,y,npts,btime,dt,maxpts,swap,nerr)
	character infile*(*)
	real y(maxpts)
	integer npts
	real btime
...
c-----
	call brsac(1,maxpts,infile,y,swap,nerr)
